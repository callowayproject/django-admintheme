...
window.SelectFilter = {
    init: function(field_id, field_name, is_stacked, admin_static_prefix) {
        ...
        var search_selector_img = quickElement('span', search_filter_label, '', 'class', 'fontawesome help-tooltip', 'title', interpolate(gettext("Type into this box to filter down the list of available %s."), [field_name]));
        search_selector_img.appendChild(document.createTextNode('\uf002'));
        filter_p.appendChild(document.createTextNode(' '));

        var filter_input = quickElement('input', filter_p, '', 'type', 'search', 'placeholder', gettext("Filter"));
        ...
        if (!is_stacked) {
            ...
            // Move the add another button, if there, to the end of the selector widget
            $(selector_div.parentNode).children('.add-another').each(function(i, elem) {
                selector_div.appendChild(elem);
            });
        }
